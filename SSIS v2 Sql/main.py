...
def student_list_p():
    ...
    def search_p():
        ...
        if srch_entry.get() == "":
            ...
            frm = Frame(root3)
            frm.pack(side=tk.LEFT, padx=5, pady=(60, 10))
    ...
    def coursel():
        ...
        #delete button
        delete_more_btn = Button(root4, bg="#161618", text="DELETE", fg="white", width=9, command=delete)
        delete_more_btn.place(x=65, y=80)

        # Update button
        update_btn = Button(root4, bg="#161618", text="UPDATE", fg="white", width=9, command=updates)
        update_btn.place(x=155, y=80)

        # Select Button
        slc_btn = Button(root4, bg="#161618", text="SELECT", fg="white", width=9, command=select)
        slc_btn.place(x=255, y=80)

        course_code = Entry(root4, width=10)
        course_code.place(x=100, y=130)

        courses = Entry(root4, width=30)
        courses.place(x=65, y=160)

        ex = Label(root4, text="List of Courses", font=("Helvitica", 18, "bold"), bg="#161618", fg="gold")
        ex.place(x=110, y=5)

        cc_lbl = Label(root4, bg="#161618", fg='white', text="Course Code:", font=('helvetica', 10))
        cc_lbl.place(x=13, y=130)

        c_lbl = Label(root4, bg="#161618", text="Course:", fg='white', font=('helvetica', 10))
        c_lbl.place(x=13, y=160)
        ...
        frm = Frame(root4)
        frm.pack(side=tk.LEFT, padx=5, pady=(180, 0))

        tv = ttk.Treeview(frm, columns=(1, 2), show="headings", height="8")
        tv.pack()

        vsb = ttk.Scrollbar(root4, orient="vertical", command=tv.yview)
        vsb.place(x=380 + 190 + 2, y=188, height=300)
    ...
    register_btn.place(x=465, y=90)


    cc = Button(root2, text="Course Register", bg="yellow", borderwidth=3,
                          activebackground="#161618", command=cr)
    cc.place(x=465, y=120)

    courselist_btn = Button(root2,text="COURSE LIST", bg="black", fg="white", command=coursel)
    courselist_btn.place(x=580, y=120)
